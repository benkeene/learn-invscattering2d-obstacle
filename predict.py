...
def parse_args():
    ...
    parser.add_argument(
        "--data_path", default="./data/star3_kh10_n48_100/temp.mat", type=str
    )
    parser.add_argument(
        "--model_path", default="./data/star3_kh10_n48_100/test", type=str
    )
...
def main():
    ...
    if train_cfg["network_type"] == 'convnet':
        data_to_predict = (
            uscat_all.real[:, None, :, :] - mean
        ) / std  # the scattered data
        ...
        data_to_predict = np.concatenate(
            (data_real[:, None, :, :], data_imag[:, None, :, :]), axis=1
        )

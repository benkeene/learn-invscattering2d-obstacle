    ...
    if args.retrain:
        ...
        if ncstr.startswith("star"):
            try:
                ...
                logger.error(
                    "cannot get the default training config path from dirname."
                )
                raise
...
def main():
    ...
    if args.cfg_by_nc:
        ...
        if (
            data_cfg["fc_max"] != 0.1
            or data_cfg["n_tgt"] != 100
            or data_cfg["n_dir"] != 100
        ):
    ...
    if network_type == "convnet":
        ...
        logger.info(
            "model complexnet, mean and std for real %.8e %.8e, imag %.8e %.8e",
            mean_r,
            std_r,
            mean_i,
            std_i,
    ...
    if ndata == 0:
        ...
        logger.warning(
            "ndata_train={:3} out numbers the data_set, will train with ndata={:3}".format(
                ndata, ndata_avail
            )
        )
        ndata = ndata_avail
    ...
    data_all = pool.map(
        ...
        [
            temp_dir
            + str((mat_id - 1) * ndata_per_mat + 1)
            + "-"
            + str(mat_id * ndata_per_mat)
            + ".mat"
    ...
    if train_cfg["n_dir_train"] > 0:
        logger.info(
            "training using partial data: num incident direction {:3}".format(
                train_cfg["n_dir_train"]
            )
        )
        uscat_all = uscat_all[:, 0 : train_cfg["n_dir_train"], :]
    if train_cfg["n_tgt_train"] > 0:
        logger.info(
            "training using partial data: num scattered direction {:3}".format(
                train_cfg["n_tgt_train"]
            )
        )
    ...
    if network_type == "convnet":
        ...
        del uscat_all
        data_to_train = np.concatenate(
            (
                (u_fft.real[:, None, :, :] - mean_r) / std_r,
                (u_fft.imag[:, None, :, :] - mean_i) / std_i,
            ),
            axis=1,
        )
        del u_fft
    ...
    del data_to_train
    logger.info(
        "successfully load training data, ndata {:3}, time: {:.1f}s".format(
            ndata, time.time() - start_time
        )
    )
    ...
    def train(model, device, train_loader, optimizer, epoch, scheduler, model_dir):
        ...
        for e in range(epoch + 1):
            ...
            if e % train_cfg["valid_freq"] == 0:
                ...
                train_logger.info(
                    "{:3}, train Loss: {:.6f}, val loss: {:.6f}, rel err: {:.4f}, abs err: {:.4f}, time: {:.1f}s".format(
                        e,
                        loss_train,
                        loss_val,
                        error_rel,
                ...
                writer.add_scalar(
                    "log_log_loss_train", np.log(loss_train), np.log(e + 1) * 1000
                )
                writer.add_scalar(
                    "log_log_loss_val", np.log(loss_val), np.log(e + 1) * 1000
                )
            if (
                train_cfg["save_every_nepoch"] > 0
                and e % train_cfg["save_every_nepoch"] == 0
                and e > 0
            ):
                torch.save(
                    model.state_dict(),
                    os.path.join(model_dir, "model_" + str(e) + ".pt"),
                )
            scheduler.step()
        logger.info(
            "final rel err {:.4f}, abs err {:.4f}".format(
                final_error_rel, final_error_abs
            )
        )
        return

    # Prefer MPS for float32 on Apple Silicon, no float64 support :(
    ...
    if args.retrain:
        ...
        model.load_state_dict(
            torch.load(os.path.join(args.dirname, args.retrain), map_location=device)
        )
    ...
    scheduler = MultiStepLR(
        optimizer, milestones=train_cfg["milestones"], gamma=train_cfg["gamma"]
    )
